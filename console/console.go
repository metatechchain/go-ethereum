...
func (c *Console) initExtensions() error {
	...
	aliases := map[string]struct{}{"mtc": {}, "personal": {}}
	for api := range apis {
		if api == "web3" {
			continue
		}
...
func (c *Console) Welcome() {
	message := "Welcome to the Metatech JavaScript console!\n\n"

	// Print some generic Geth metadata
	if res, err := c.jsre.Run(`
		var message = "instance: " + web3.version.node + "\n";
		try {
			message += "coinbase: " + mtc.coinbase + "\n";
		} catch (err) {}
		message += "at block: " + mtc.blockNumber + " (" + new Date(1000 * mtc.getBlock(mtc.blockNumber).timestamp) + ")\n";
		try {
